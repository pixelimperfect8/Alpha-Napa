...
// COBE shader J(theta,phi) maps screen→world; we use its transpose for world→screen.
// Sphere radius in NDC = sqrt(0.64) = 0.8, so GLOBE_SCALE = 0.8 * 50 = 40.
function projectMarker(lat: number, lng: number, phi: number, theta: number) {
    const latRad = (lat * Math.PI) / 180;
    const a = (lng * Math.PI) / 180 - Math.PI; // COBE subtracts PI from longitude

    const cosLat = Math.cos(latRad);
    // COBE world coords: e = [-cos(lat)*cos(a), sin(lat), cos(lat)*sin(a)]
    ...
    // world→screen = world * J^T (transpose of COBE's rotation matrix)
    const sx = wx * cp + wz * sp;
    const sy = wx * sp * st + wy * ct - wz * cp * st;
    const sz = -wx * sp * ct + wy * st + wz * cp * ct;

    return { x: sx, y: -sy, visible: sz > 0 }; // negate y for CSS (top-down)
}
...
export default function AlphaCampuses() {
    ...
    useEffect(() => {
        ...
        import("cobe").then(({ default: createGlobe }) => {
            ...
            globe = createGlobe(canvasRef.current, {
                ...
                markers,
                onRender: (state) => {
                    // If dragging, apply pointer movement
                    if (pointerInteracting.current !== null) {
                        // No auto-rotation during drag — user controls phi
                    } else if (targetPhiRef.current !== null) {
                        // Smooth lerp toward target city
                    ...
                    // Update overlay dot positions via DOM refs
    ...
    return (
        <section
            ref={containerRef}
            id="alpha-campuses"
            className="relative h-screen w-full overflow-hidden bg-[#FDFBF7] rounded-b-[2.5rem]"
        >
            <motion.div
                style={{ y }}
